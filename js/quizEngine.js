...
export class QuizEngine {
  ...
  formatUserAnswer(question, answer) {
    ...
    if (question.type === 'boolean') return answer ? 'Sant' : 'Falskt'
    if (question.type === 'single') return question.options[answer] ?? '(Ogiltigt svar)'
    if (question.type === 'multiple') return answer.map(i => question.options[i]).join(', ')
  ...
  calculateResults() {
    ...
    return { correctCount, total, percentage, details }
  }
}
